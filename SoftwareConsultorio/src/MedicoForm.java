...
public class MedicoForm extends JFrame {
	...
	public void buscarMedicos() {
		try {
			int q;
			String sql = "SELECT m.id as id, m.nome as nome, m.crm as crm, e.nome_especializacao as especializacao FROM medicos m JOIN especializacoes e ON m.especializacao = e.id;";
			cx.pst = cx.con.prepareStatement(sql);
			cx.rs = cx.pst.executeQuery();
	...
	public void camposAtivadosBtnNovo() {
	...
	public void camposAtivadosSelecionarCampoTabela() {
		txtNome.setEnabled(true);
	    txtCRM.setEnabled(true);
	    txtTelefone.setEnabled(true);
	    txtEmail.setEnabled(true);
	...
	public MedicoForm() {
		...
		ImageIcon imgMed = new ImageIcon(getClass().getResource("/img/equipe-medica.png"));
			...
			btnEditar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					try {
						...
						String sql = "UPDATE medicos SET nome=?, crm=?, especializacao=?, telefone=?, whatsapp=?, email=? WHERE id = ?;";
						cx.pst = cx.con.prepareStatement(sql);
				...
				public void mouseClicked(MouseEvent e) {
					try {
						if(table.getSelectedRow() > -1) {
							...
							String sql = "SELECT m.nome as nome, m.crm as crm, m.telefone as telefone, m.whatsapp as whatsapp, m.email as email, m.especializacao as id_especializacao ,e.nome_especializacao as especializacao FROM medicos m JOIN especializacoes e ON m.especializacao = e.id WHERE m.id = ?";
							cx.pst = cx.con.prepareStatement(sql);
